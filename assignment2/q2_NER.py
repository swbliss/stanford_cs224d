...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.input_placeholder = tf.placeholder(
        dtype=tf.int32, shape=(None, self.config.window_size))
    self.labels_placeholder = tf.placeholder(
      dtype=tf.float32, shape=(None, self.config.label_size))
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope("NER"):
        L = tf.get_variable(
          "L", (len(self.wv), self.config.embed_size),
          initializer=tf.constant_initializer(),
        )

      embeds = tf.nn.embedding_lookup(L, tf.reshape(self.input_placeholder, [-1]))
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):
      ...
      b1 = tf.get_variable(
        "b1", shape=[self.config.hidden_size,], initializer=xavier_weight_init()
      )
      tf.nn.dropout(W, self.config.dropout)
      tf.nn.dropout(b1, self.config.dropout)

    with tf.variable_scope("Softmax"):
      U = tf.get_variable(
        "U", shape=[self.config.hidden_size, self.config.label_size],
        initializer=xavier_weight_init()
      )
      b2 = tf.get_variable(
        "b2", shape=[self.config.label_size], initializer=xavier_weight_init()
      )
      tf.nn.dropout(U, self.config.dropout)
      tf.nn.dropout(b2, self.config.dropout)
  ...
  def add_loss_op(self, y):
    ...
    with tf.variable_scope("Layer"):
      ...
      regularization += tf.reduce_sum(tf.square(tf.get_variable("W")))
    with tf.variable_scope("Softmax"):
      tf.get_variable_scope().reuse_variables()
      regularization += tf.reduce_sum(tf.square(tf.get_variable("U")))
    regularization = regularization * self.config.l2 / 2.0
