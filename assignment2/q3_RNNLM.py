...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      L = tf.get_variable(
        "L", shape=[len(self.vocab), self.config.embed_size],
        initializer=tf.constant_initializer()
      )
      embeds = tf.nn.embedding_lookup(L, self.input_placeholder)
      inputs = [
        tf.squeeze(x, [1]) for x in tf.split(1, self.config.num_steps, embeds)]
      ### END YOUR CODE
      return inputs
  ...
  def add_loss_op(self, output):
    ...
    # loss_weights = [tf.ones([self.config.batch_size * self.config.num_steps])]
    # loss = tf.nn.seq2seq.sequence_loss(
    #   output, [tf.reshape(self.labels_placeholder, [-1])], loss_weights, len(self.vocab))
    ### END YOUR CODE
    return loss
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN"):
      H = tf.get_variable(
        "H", shape=[self.config.hidden_size, self.config.hidden_size],
        initializer=tf.constant_initializer()
      )
      I = tf.get_variable(
        "I", shape=[self.config.embed_size, self.config.hidden_size],
        initializer=tf.constant_initializer()
      )
      b_1 = tf.get_variable(
        "b_1", shape=[self.config.hidden_size,],
        initializer=tf.constant_initializer()
      )

    prev_h = tf.zeros([self.config.batch_size, self.config.hidden_size])
    rnn_outputs = []

    with tf.variable_scope("RNN"):
      tf.get_variable_scope().reuse_variables()
      for input in inputs:
        prev_h = tf.nn.sigmoid(tf.matmul(prev_h, H) + tf.matmul(input, I) + b_1)
